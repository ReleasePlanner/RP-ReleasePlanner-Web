...
describe('authService', () => {
  ...
  describe('login', () => {
    it('should login and store tokens and user', async () => {
      ...
      expect(localStorage.getItem(USER_STORAGE_KEY)).toBe(JSON.stringify(mockResponse.user));
    });
  });
  ...
  describe('refreshToken', () => {
    it('should refresh token and update storage', async () => {
      ...
      expect(localStorage.getItem(REFRESH_TOKEN_STORAGE_KEY)).toBe('new-refresh-token');
    });
  });
  ...
  describe('getCurrentUser', () => {
    it('should get current user from API', async () => {
      ...
      expect(httpClient.post).toHaveBeenCalledWith('/auth/me', undefined);
    });
  });
